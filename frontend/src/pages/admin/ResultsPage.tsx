...
export default function ResultsPage() {
  ...
  const convertToCSV = (data: ResultRow[]) => {
    const headers = ['Tên Thí Sinh', 'CCCD', 'Lớp', 'Kỳ Thi', 'Môn Thi', 'Điểm', 'Trạng Thái']
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{(isAdmin) ? 'Quản Lý Kết Quả' : 'Kết Quả Thi'}</h1>
        ...
        {(isTeacher && results.length > 0) && (
          <div className="flex gap-3">
            <button className="btn-primary" onClick={fetchData}><FiRefreshCcw/> Tải lại kết quả</button>
            <button className="btn-secondary" onClick={downloadResults}><FiDownload /> Tải xuống kết quả</button>
          </div>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                {(isAdmin || isTeacher) && <th className="text-left py-3 px-4">Thí sinh</th>}
              ...
              {results.map((result, index) => (
                <tr key={result.result_id || index} className="border-b hover:bg-gray-50">
                  ...
                  {isTeacher && (
                    <td className="py-3 px-4">
                      <div className="font-medium">{result.student_name}</div>
                      <div className="text-xs text-gray-500">{result.student_cccd} - {result.class_name}</div>
                    </td>
                  )}
