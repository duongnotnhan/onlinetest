              ...
              <button
                onClick={() => {
                  setTargetSlot({
                    ...
                    order,
                    studentSchoolId: response.student_school_id,
                  });
                  setSelectedGraderId(grade.grader_id.toString());
                  setShowModal(true);
                }}
                className="p-1 bg-white border rounded hover:text-blue-600"
              >
                <FiEdit size={12} />
              </button>
              <button
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Điều phối và gán bài tự luận cho giám khảo Ngữ Văn.
          </p>
        </div>
        <select
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-100 border-b">
              <tr>
                ...
                <th className="py-3 px-4 font-bold text-gray-700 w-48 text-center">
                  Giám khảo 1
                </th>
                <th className="py-3 px-4 font-bold text-gray-700 w-48 text-center">
                  Giám khảo 2
                ...
                <tr key={a.response_id} className="hover:bg-gray-50">
                  <td className="py-3 px-4">
                    ...
                    <p className="text-gray-500 font-medium">
                      Câu tự luận số {a.question_number} - Của CCCD:{" "}
                      {a.student_name}
                    </p>
                  </td>
              ...
              {assignments.length === 0 && sessionId && (
                <tr>
                  <td colSpan={4} className="py-12 text-center text-gray-500">
                    Chưa có bài tự luận nào được nộp trong kỳ thi này.
                  </td>
                </tr>
              )}
              {!sessionId && (
                <tr>
                  ...
                  >
                    Vui lòng chọn kỳ thi để xem phân công.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleAssign} className="p-6">
              ...
              >
                ...
                {gradersList.map((g) => {
                  const isSameSchool =
                    g.school_id === targetSlot.studentSchoolId;
                  return (
                    <option
                      key={g.user_id}
                      value={g.user_id}
                      className={
                        isSameSchool ? "text-red-500 font-bold bg-red-50" : ""
                      }
                    >
                      {g.full_name} ({g.school_name}){" "}
                      {isSameSchool ? "Trùng trường" : ""}
                    </option>
                  );
                })}
              </select>
