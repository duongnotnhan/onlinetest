...
import { FiUpload, FiRefreshCw, FiXCircle, FiCheckCircle, FiAlertCircle, FiEdit } from 'react-icons/fi'
...
export default function TeacherStudentsPage() {
  ...
  const resetPassword = async (studentId: number) => {
    if (!confirm('Bạn có chắc chắn muốn cấp lại mật khẩu tạm thời cho thí sinh này?')) return
    try {
      const response = await teacherAPI.resetStudentPassword(studentId)
      toast.success(`Mật khẩu mới: ${response.data.temporary_password}`, { duration: 6000 })
    } catch (error: any) {
  ...
  const handleImport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.failed === 0) {
        ...
        setTimeout(() => { setShowImportModal(false); setImportFile(null); setImportResult(null) }, 2000)
      } else {
        toast.error(`Nhập hoàn tất. Thành công: ${response.data.imported}, Lỗi: ${response.data.failed}`)
      }
      fetchStudents()
    } catch (error: any) {
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    if (editForm.elective_subject_1 === editForm.elective_subject_2 && editForm.elective_subject_1 !== '') {
  ...
  if (loading) return <div className="text-center py-8">Đang tải dữ liệu hệ thống...</div>
    ...
    <div className="space-y-6 text-left">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div>
          ...
          <p className="text-gray-500 mt-1">Quản lý hồ sơ, cấp lại mật khẩu và nhập danh sách dự thi.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setShowImportModal(true)} className="btn-primary flex items-center gap-2 text-sm">
      ...
      <div className="card space-y-4 p-0 overflow-hidden">
        <div className="flex justify-between items-center border-b pb-4 bg-gray-900 text-white p-4">
          ...
          <span className="bg-blue-600 px-3 py-1 rounded text-sm font-medium">{students.length} Thí sinh</span>
        </div>
        <div className="overflow-x-auto p-4">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50 text-gray-700">
                <th className="text-left py-3 px-4 font-bold uppercase tracking-wider text-xs">Họ và tên</th>
                <th className="text-left py-3 px-4 font-bold uppercase tracking-wider text-xs">Mã định danh</th>
                <th className="text-left py-3 px-4 font-bold uppercase tracking-wider text-xs">Lớp</th>
                <th className="text-left py-3 px-4 font-bold uppercase tracking-wider text-xs">Môn Tự Chọn</th>
                <th className="text-right py-3 px-4 font-bold uppercase tracking-wider text-xs">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {students.map((student) => (
                <tr key={student.student_id} className="border-b hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-3 px-4 font-mono text-blue-600">{student.cccd}</td>
                  <td className="py-3 px-4">{student.class_name}</td>
                  <td className="py-3 px-4">
                    {student.elective_subject_1 || student.elective_subject_2 ? (
                      <div className="flex flex-col gap-1 text-xs">
                        <span className="bg-gray-100 px-2 py-1 rounded w-fit">{ELECTIVE_SUBJECTS.find(s => s.code === student.elective_subject_1)?.name || student.elective_subject_1}</span>
                        <span className="bg-gray-100 px-2 py-1 rounded w-fit">{ELECTIVE_SUBJECTS.find(s => s.code === student.elective_subject_2)?.name || student.elective_subject_2}</span>
                      </div>
                    ) : <span className="text-red-500 text-xs italic">Chưa chọn môn</span>}
                  </td>
              ...
              {students.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center py-12 text-gray-500">
                    Trường chưa có thí sinh nào. Vui lòng nhập dữ liệu từ file CSV.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col overflow-hidden">
             <div className="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
                <h3 className="text-xl font-bold text-gray-900">Sửa thông tin thí sinh</h3>
                <button onClick={() => setShowEditModal(false)} className="text-gray-400 hover:text-red-500"><FiXCircle size={24}/></button>
             </div>
             <form id="edit-form" onSubmit={handleSaveEdit} className="p-6 space-y-4">
                <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className="input-field" value={editForm.class_name} onChange={e => setEditForm({...editForm, class_name: e.target.value})} required />
                  </div>
                  <div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mt-4">
                  <p className="font-bold text-blue-900 mb-3 text-sm">Đăng ký môn thi (Mặc định: Toán, Ngữ Văn)</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Môn Tự chọn 1</label>
                      <select className="input-field py-2" value={editForm.elective_subject_1} onChange={e => setEditForm({...editForm, elective_subject_1: e.target.value})} required>
                        <option value="">-- Chọn môn 1 --</option>
                        {ELECTIVE_SUBJECTS.map(sub => <option key={`1-${sub.code}`} value={sub.code}>{sub.name}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Môn Tự chọn 2</label>
                      <select className="input-field py-2" value={editForm.elective_subject_2} onChange={e => setEditForm({...editForm, elective_subject_2: e.target.value})} required>
                        <option value="">-- Chọn môn 2 --</option>
                        {ELECTIVE_SUBJECTS.map(sub => <option key={`2-${sub.code}`} value={sub.code}>{sub.name}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
             ...
             <div className="px-6 py-4 border-t bg-gray-50 flex justify-end gap-3">
                ...
                <button type="submit" form="edit-form" className="btn-primary" disabled={saving}>{saving ? 'Đang lưu...' : 'Lưu thay đổi'}</button>
             </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl flex flex-col overflow-hidden">
             <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
                <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2"><FiUpload /> Nhập danh sách thí sinh</h3>
                <button onClick={() => setShowImportModal(false)} className="text-gray-400 hover:text-red-500"><FiXCircle size={24}/></button>
             </div>
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-3 gap-4 text-center">
                       <div className="bg-gray-50 border p-4 rounded-lg">
                          <p className="text-sm text-gray-500 font-medium">Tổng số dòng</p>
                          <p className="text-2xl font-bold text-gray-900">{importResult.total}</p>
                       </div>
                       <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
                          <p className="text-sm text-green-700 font-medium flex items-center justify-center gap-1"><FiCheckCircle/> Thành công</p>
                          <p className="text-2xl font-bold text-green-700">{importResult.imported}</p>
                       </div>
                       <div className="bg-red-50 border border-red-200 p-4 rounded-lg">
                          <p className="text-sm text-red-700 font-medium flex items-center justify-center gap-1"><FiAlertCircle/> Lỗi</p>
             ...
             <div className="px-6 py-4 border-t bg-gray-50 flex justify-end gap-3">
                <button type="button" className="btn-secondary" onClick={() => { setShowImportModal(false); setImportFile(null); setImportResult(null) }}>
                  {importResult ? 'Đóng' : 'Hủy'}
                </button>
                {!importResult && (
                  <button type="submit" form="import-form" className="btn-primary flex items-center gap-2" disabled={importing || !importFile}>
                    {importing ? 'Đang xử lý...' : <><FiUpload /> Tiến hành Import</>}
                  </button>
                )}
             </div>
          </div>
