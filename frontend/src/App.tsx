...
function ProtectedRoute({ children, allowedRoles, hideLayout = false }: ProtectedRouteProps) {
  ...
  if ((user?.role === 'admin' || user?.role === 'teacher') && !user?.two_fa_enabled) {
...
function RootRedirect() {
  const { isAuthenticated, user } = useAuthStore()
  return <Navigate to={isAuthenticated ? nextRouteForUser(user) : '/login'} replace />
}
...
function nextRouteForUser(user: any) {
  ...
  if ((user.role === 'admin' || user.role === 'teacher') && !user.two_fa_enabled) return '/setup-2fa'
