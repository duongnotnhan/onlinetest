...
from app.models import (
    User, ExamPaper, Question, AnswerChoice, Answer, GeneratedPaper, QuestionItem,
    Subject, ExamSession, ExamSchedule, ExamAttempt, StudentResponse, Student, EssayGrade, Teacher
)
...
def create_exam_paper():
    ...
    try:
        ...
        if not data.get('subject_id') or not data.get('exam_session_id') or not data.get('paper_code'):
        ...
        if paper:
            ...
            # For Vietnamese Language (Ngữ Văn), set shared reading material if provided
...
def get_exam_papers():
    ...
    try:
        ...
        total = query.count()
        papers = query.order_by(ExamPaper.created_at.desc()).paginate(page=page, per_page=limit, error_out=False)
        ...
        for paper in papers.items:
            ...
            data.append({
                ...
                'session_name': ExamSession.query.get(paper.exam_session_id).session_name if paper.exam_session_id else None,
...
def add_question(paper_id):
    ...
    try:
        ...
        if not data.get('question_type') or not data.get('question_text') or not data.get('part'):
            return jsonify({'error': 'Missing required fields'}), 400
        ...
        success, result = ExamPaperService.add_question(
            paper_id=paper_id,
            question_type=data['question_type'],  # 'multiple_choice', 'essay', 'short_answer'
...
def add_answer_choice(question_id):
    ...
    try:
        ...
        if not data.get('choice_label') or not data.get('choice_text') or not data.get('display_order'):
            return jsonify({'error': 'Missing required fields'}), 400
...
def delete_question(question_id):
    ...
    try:
        ...
        if paper and paper.is_finalized:
            return jsonify({'error': 'Cannot delete questions from finalized papers'}), 400
...
def submit_response(attempt_id):
    ...
    try:
        ...
        exam_datetime_end = attempt.end_time or datetime.combine(schedule.exam_date, schedule.end_time)
        ...
        if data.get('selected_informatics_track') in ['computer_science', 'applied_informatics']:
            attempt.selected_informatics_track = data['selected_informatics_track']

        if question.question_type == 'short_answer' and not _is_valid_four_cell_answer(data['student_answer']):
            return jsonify({'error': 'Short answer may contain only digits, minus sign, and comma, up to 4 cells'}), 400
...
def submit_exam(attempt_id):
    ...
    try:
        ...
        success, score_result = ExamScoringService.auto_score_multiple_choice(attempt_id)
        has_essay = db.session.query(Question).join(
            StudentResponse, Question.question_id == StudentResponse.question_id
        ).filter(
...
def _assign_essay_graders(attempt):
    ...
    eligible = [teacher.user_id for teacher in teachers if teacher.school_id != attempt.student.school_id]
    if len(eligible) < 2:
...
def add_subsection(paper_id):
    ...
    try:
        ...
        section_type = QuestionSectionType.query.filter_by(section_type_key=type_key).first()
        if not section_type:
            section_type = QuestionSectionType(section_type_name=type_key, section_type_key=type_key)
        ...
        # Get next order
        last_sec = QuestionSection.query.filter_by(paper_id=paper_id).order_by(QuestionSection.display_order.desc()).first()
        ...
        section = QuestionSection(
            ...
            section_description=json.dumps(meta), # Lưu data xáo trộn và ngữ liệu vào description
...
def import_questions_csv(paper_id):
    ...
    try:
        ...
        for row in csv_data:
            ...
            track_val = None
            if track.lower() in ['cs', 'computer_science', 'khoa học máy tính']: track_val = 'computer_science'
            elif track.lower() in ['ict', 'applied_informatics', 'tin học ứng dụng']: track_val = 'applied_informatics'
            elif track.lower() in ['common', 'chung']: track_val = 'common'
            ...
            if question_type == 'multiple_choice':
                ...
                ExamPaperService.add_question(
                    paper_id=paper_id, question_type=question_type, question_text=text,
                    part=part, question_number=current_q_num, points=points, created_by=user_id,
                    choices=choices, answer_value=correct, informatics_track=track_val
                )
                imported += 1
                
            elif question_type == 'true_false':
                items = [
                    {'label': 'a', 'text': row.get('Y_a', '').strip(), 'correct_value': 'true' if row.get('DS_a', '').strip().lower() in ['đ', 'đúng', 'true', '1'] else 'false'},
                    {'label': 'b', 'text': row.get('Y_b', '').strip(), 'correct_value': 'true' if row.get('DS_b', '').strip().lower() in ['đ', 'đúng', 'true', '1'] else 'false'},
                    {'label': 'c', 'text': row.get('Y_c', '').strip(), 'correct_value': 'true' if row.get('DS_c', '').strip().lower() in ['đ', 'đúng', 'true', '1'] else 'false'},
                    {'label': 'd', 'text': row.get('Y_d', '').strip(), 'correct_value': 'true' if row.get('DS_d', '').strip().lower() in ['đ', 'đúng', 'true', '1'] else 'false'}
                ]
                ExamPaperService.add_question(
                    paper_id=paper_id, question_type=question_type, question_text=text,
                    part=part, question_number=current_q_num, points=points, created_by=user_id,
                    items=items, informatics_track=track_val
                )
                imported += 1
                ...
                ExamPaperService.add_question(
                    paper_id=paper_id, question_type=question_type, question_text=text,
                    part=part, question_number=current_q_num, points=points, created_by=user_id,
                    answer_value=correct, informatics_track=track_val
                )
                imported += 1
...
def reorder_questions(paper_id):
    ...
    try:
        ...
        # Dùng session.no_autoflush để tránh SQLAlchemy tự động đẩy câu lệnh lỗi lên DB sớm
