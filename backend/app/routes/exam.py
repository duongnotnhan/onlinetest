...
def create_exam_paper():
    ...
    try:
        ...
        if (
            not data.get("subject_id")
            or not data.get("exam_session_id")
            or not data.get("paper_code")
        ):
            ...
            data.append(
                {
                    ...
                    "session_name": (
                        ExamSession.query.get(paper.exam_session_id).session_name
                        if paper.exam_session_id
                        else None
                    ),
...
def add_question(paper_id):
    ...
    try:
        ...
        if (
            not data.get("question_type")
            or not data.get("question_text")
            or not data.get("part")
        ):
            return jsonify({"error": "Missing required fields"}), 400
...
def add_answer_choice(question_id):
    ...
    try:
        ...
        if (
            not data.get("choice_label")
            or not data.get("choice_text")
            or not data.get("display_order")
        ):
            return jsonify({"error": "Missing required fields"}), 400
...
def delete_question(question_id):
    ...
    try:
        ...
        if paper and paper.is_finalized:
            return (
                jsonify({"error": "Cannot delete questions from finalized papers"}),
                400,
            )
...
def _assign_essay_graders(attempt):
    ...
    eligible = [
        ...
        if teacher.school_id != attempt.student.school_id
    ]
    if len(eligible) < 2:
        ...
        for row in csv_data:
            ...
            if question_type == "multiple_choice":
                ...
                items = [
                    {
                        "label": "a",
                        "text": row.get("Y_a", "").strip(),
                        "correct_value": (
                            "true"
                            if row.get("DS_a", "").strip().lower()
                            in ["đ", "đúng", "true", "1"]
                            else "false"
                        ),
                    },
                    {
                        "label": "b",
                        "text": row.get("Y_b", "").strip(),
                        "correct_value": (
                            "true"
                            if row.get("DS_b", "").strip().lower()
                            in ["đ", "đúng", "true", "1"]
                            else "false"
                        ),
                    },
                    {
                        "label": "c",
                        "text": row.get("Y_c", "").strip(),
                        "correct_value": (
                            "true"
                            if row.get("DS_c", "").strip().lower()
                            in ["đ", "đúng", "true", "1"]
                            else "false"
                        ),
                    },
                    {
                        "label": "d",
                        "text": row.get("Y_d", "").strip(),
                        "correct_value": (
                            "true"
                            if row.get("DS_d", "").strip().lower()
                            in ["đ", "đúng", "true", "1"]
                            else "false"
                        ),
                    },
