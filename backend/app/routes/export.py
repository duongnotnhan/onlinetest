...
from app import db
from app.models import User, Student, ExamAttempt, Subject, ExamSession, School
...
# ========================================================
@export_bp.route('/data/<string:entity_type>/<string:export_type>', methods=['POST', 'GET'])
@jwt_required()
    ...
    try:
        ...
        if entity_type == 'students':
            if current_user.role == 'admin':
                headers = ['ID Tỉnh/Thành', 'ID Phường/Xã', 'Tỉnh/Thành', 'Phường/Xã', 'Trường Học', 'Mã Học Sinh', 'Họ và Tên', 'Mã Định Danh', 'Giới Tính', 'Ngày Sinh', 'Lớp', 'Địa Chỉ Chi Tiết']
                filename = "Danh_Sach_Hoc_Sinh_Toan_He_Thong"
                
                try:
                    students = db.session.query(Student, School).outerjoin(School, Student.school_id == School.school_id)\
                                 .order_by(Student.province_id, Student.ward_id, School.school_name, Student.class_name, Student.full_name).all()
                except Exception:
                    db.session.rollback()
                    students = db.session.query(Student, School).outerjoin(School, Student.school_id == School.school_id)\
                                 .order_by(School.school_name, Student.class_name, Student.full_name).all()

                for stu, sch in students:
                    prov_id = getattr(stu, 'province_id', getattr(sch, 'province_id', ''))
                    ward_id = getattr(stu, 'ward_id', getattr(sch, 'ward_id', ''))
                    prov_name = getattr(stu.province, 'name', prov_id) if hasattr(stu, 'province') else prov_id
                    dist_name = getattr(stu.district, 'name', getattr(stu, 'district_id', '')) if hasattr(stu, 'district') else ''
                    
                    data_rows.append([
                        prov_id, ward_id, prov_name, dist_name,  
                        sch.school_name if sch else 'Chưa gán trường',
                        stu.student_code or '', stu.full_name or '', stu.cccd or '',
                        'Nam' if stu.gender == 'male' else 'Nữ', str(stu.date_of_birth) if stu.date_of_birth else '',
                        stu.class_name or '', stu.address or ''
                    ])

            elif current_user.role == 'teacher':
                school_id = get_school_id_for_user(current_user)
                headers = ['Mã Học Sinh', 'Họ và Tên', 'CCCD/Mã Định Danh', 'Giới Tính', 'Ngày Sinh', 'Lớp Học', 'Số Điện Thoại', 'Địa Chỉ']
                ...
                for s in students:
                    data_rows.append([
                        s.student_code or '', s.full_name or '', s.cccd or '',
                        'Nam' if s.gender == 'male' else 'Nữ', str(s.date_of_birth) if s.date_of_birth else '',
                        s.class_name or '', s.phone or '', s.address or ''
                    ])
        ...
        # -----------------------------------------------
            ...
            session_id = req_data.get('exam_session_id') or request.args.get('exam_session_id')
            ...
            headers = ['Số Thứ Tự', 'Mã Học Sinh', 'Họ và Tên', 'Mã định danh', 'Ngày sinh', 'Giới tính', 'Lớp', 'Trường', 'Môn Thi', 'Điểm Số', 'Trạng Thái']
            filename = f"Ket_Qua_Thi_Ky_Thi_{session_id}"
            
            query = db.session.query(ExamAttempt, Student, Subject)\
            ...
            if current_user.role == 'teacher':
                query = query.filter(Student.school_id == get_school_id_for_user(current_user))
            ...
            for idx, (att, stu, subj) in enumerate(attempts, start=1):
                data_rows.append([
                    idx, stu.student_code or '', stu.full_name, stu.cccd, str(stu.date_of_birth) if stu.date_of_birth else '', 'Nam' if stu.gender == 'male' else 'Nữ', stu.class_name or '', stu.school.name if stu.school else '',
                    subj.subject_name if subj else f"Mã: {att.subject_id}",
                    float(att.total_score) if att.total_score is not None else 0.0,
                    'Đã hoàn thành' if att.status in ['graded', 'completed'] else 'Chưa nộp bài hoặc bài nộp không hợp lệ'
                ])
        ...
        # -----------------------------------------------
            ...
            headers = ['Số Thứ Tự', 'Mã Định Danh', 'Họ và Tên', 'Ngày Sinh', 'Giới Tính', 'Lớp', 'Mã Môn Thi', 'Tên Môn Thi', 'Điểm', 'Kỳ Thi', 'Trạng Thái']
            filename = f"Ket_Qua_Thi_Cua_Truong"
            
            query = db.session.query(ExamAttempt, Student, Subject, ExamSession)\
                      .join(Student, ExamAttempt.student_id == Student.student_id)\
                      .outerjoin(Subject, ExamAttempt.subject_id == Subject.subject_id)\
                      .outerjoin(ExamSession, ExamAttempt.exam_session_id == ExamSession.exam_session_id)
            
            if current_user.role == 'teacher':
                school_id = get_school_id_for_user(current_user)
                query = query.filter(Student.school_id == school_id)
            else:
                return jsonify({'error': 'Chỉ giáo viên quản lý mới được phép xuất dữ liệu này'}), 403
        ...
        if export_type == 'csv':
            ...
            fill_header = PatternFill(start_color='1E3A8A', end_color='1E3A8A', fill_type='solid') # Xanh biển
