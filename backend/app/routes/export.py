...
def download_template(template_type):
    ...
    if template_type == 'student':
        ...
        writer.writerow(['NoiDung', 'Diem', 'A', 'B', 'C',
                        'D', 'DapAnDung', 'DinhHuong'])
        writer.writerow(['Đáp án nào dưới đây chỉ thủ đô của Việt Nam?',
                        '0,25', 'Hà Nội', 'TP.HCM', 'Đà Nẵng', 'Huế', 'A', 'Chung'])

    elif template_type == 'question_tf':
        filename = "Mau_Cau_Hoi_Dung_Sai.csv"
        writer.writerow(['NoiDung', 'Diem', 'Y_a', 'DS_a', 'Y_b',
                        'DS_b', 'Y_c', 'DS_c', 'Y_d', 'DS_d', 'DinhHuong'])
        writer.writerow(['Các nhận định sau đây đúng hay sai?',
                         '1,0',
...
# ========================================================
@export_bp.route('/data/<string:entity_type>/<string:export_type>',
                 methods=['POST', 'GET'])
@jwt_required()
    ...
    try:
        ...
        if entity_type == 'students':
            if current_user.role == 'admin':
                ...
                for stu, sch in students:
                    prov_id = getattr(
                        stu, 'province_id', getattr(
                            sch, 'province_id', ''))
                    ward_id = getattr(
                        stu, 'ward_id', getattr(
                            sch, 'ward_id', ''))
                    prov_name = getattr(
                        stu.province, 'name', prov_id) if hasattr(
                        stu, 'province') else prov_id
                    dist_name = getattr(
                        stu.district, 'name', getattr(
                            stu, 'district_id', '')) if hasattr(
                        stu, 'district') else ''
                ...
                for s in students:
                    data_rows.append([s.student_code or '',
                                      s.full_name or '',
                                      s.cccd or '',
        ...
        # -----------------------------------------------
            ...
            session_id = req_data.get(
                'exam_session_id') or request.args.get('exam_session_id')
        ...
        # -----------------------------------------------
            ...
            if current_user.role == 'teacher':
                ...
                return jsonify(
                    {'error': 'Chỉ giáo viên quản lý mới được phép xuất dữ liệu này'}), 403
        ...
        if export_type == 'csv':
            ...
            fill_header = PatternFill(
                start_color='1E3A8A',
                end_color='1E3A8A',
                fill_type='solid')  # Xanh biển
            font_body = Font(name='Arial', size=11)
            thin_border = Border(
                left=Side(
                    style='thin', color='E2E8F0'), right=Side(
                    style='thin', color='E2E8F0'), top=Side(
                    style='thin', color='E2E8F0'), bottom=Side(
                    style='thin', color='E2E8F0'))
