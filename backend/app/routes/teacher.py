    ...
    try:
        ...
        for session in sessions:
            session_results = db.session.query(
                ...
                ExamResult.score,
                ExamResult.status).join(
                StudentSubjectRegistration,
                Student.student_id == StudentSubjectRegistration.student_id).join(
                Subject,
                StudentSubjectRegistration.subject_id == Subject.subject_id).join(
                ExamResult,
                (ExamResult.student_id == Student.student_id) & (
                    ExamResult.subject_id == Subject.subject_id)).filter(
                        Student.school_id == school_id,
                StudentSubjectRegistration.exam_session_id == session.exam_session_id).all()
    ...
    try:
        ...
        if teacher.subject_specialty == 'NGU_VAN_GRADER':
            ...
            graded = EssayGrade.query.filter_by(
                grader_id=user_id, final_status='completed').count()
            return jsonify({
                'role_type': 'grader',
                'stats': {
        ...
        # DÀNH CHO GIÁO VIÊN QUẢN LÝ TRƯỜNG (GVQL)
            ...
            active_sessions = ExamSession.query.filter_by(
                is_published=True, is_locked=False).count()
...
def get_students():
    ...
    try:
        ...
        latest_session = ExamSession.query.order_by(
            ExamSession.created_at.desc()).first()
        ...
        for student in students.items:
            ...
            if session_id:
                regs = StudentSubjectRegistration.query.filter_by(
                    student_id=student.student_id, exam_session_id=session_id).all()
                subject_ids = [r.subject_id for r in regs]
                if subject_ids:
                    subjects = Subject.query.filter(
                        Subject.subject_id.in_(subject_ids)).all()
                    electives = [
                        s.subject_code for s in subjects if s.subject_code not in [
                            'TOAN', 'NVVAN']]
                    if len(electives) > 0:
    ...
    try:
        ...
        if 'elective_subject_1' in data or 'elective_subject_2' in data:
            ...
            if latest_session:
                ...
                for code in ['TOAN', 'NVVAN']:
                    ...
                    if subj:
                        existing_reg = StudentSubjectRegistration.query.filter_by(
                            student_id=student_id, subject_id=subj.subject_id, exam_session_id=session_id).first()
                        if not existing_reg:
                            db.session.add(
                                StudentSubjectRegistration(
                ...
                regs = StudentSubjectRegistration.query.filter_by(
                    student_id=student_id, exam_session_id=session_id).all()
                for reg in regs:
    ...
    try:
        ...
        if not exam_session_id:
            ...
            if not latest_session:
                return jsonify(
                    {'error': 'Chưa có kỳ thi nào được tạo trên hệ thống'}), 400
            ...
            try:
                ...
                if row.get('gender'):
                    if row['gender'].lower() not in [
                            'male', 'female', 'nam', 'nu', 'nữ']:
                        raise ValueError(
                            'Giới tính phải là Male, Female hoặc Nam, Nữ.')
                    gender = 'male' if row['gender'].lower() in [
                ...
                if row.get('tuchon1') == 'MT' and row.get(
                        'tuchon2') and row['tuchon2'] != 'MT':
                    raise ValueError(
...
def register_students_for_exam():
    ...
    try:
        ...
        if not data.get('exam_session_id') or not data.get(
                'student_ids') or not data.get('subject_ids'):
...
def create_makeup_request():
    ...
    try:
        ...
        if not data.get('exam_session_id') or not data.get(
                'student_ids') or not data.get('subject_id'):
            return jsonify({'error': 'Missing required fields'}), 400
