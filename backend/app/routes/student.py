...
from app.models import (
    User, Student, ExamAttempt, StudentResponse, ExamResult,
    ExamSchedule, StudentSubjectRegistration, ExamSession, Subject,
    ExamPaper, GeneratedPaper, Question, QuestionItem, AnswerChoice, Answer, QuestionSection
)
...
def get_dashboard():
    ...
    try:
        ...
        schedules = schedules_response.get_json().get('schedules', []) if status == 200 else []
...
def start_exam(schedule_id):
    ...
    try:
        ...
        if session.session_type == 'test':
            ...
            if not (exam_datetime_start - timedelta(minutes=5) <= now <= exam_datetime_end):
        ...
        generated_papers = GeneratedPaper.query.filter_by(paper_id=paper.paper_id, is_active=True).all()
...
def get_exam_paper(attempt_id):
    ...
    try:
        ...
        all_questions = Question.query.filter_by(paper_id=paper.paper_id, is_visible=True).all()
        ...
        # Fetch Subsections
        sections = QuestionSection.query.filter_by(paper_id=paper.paper_id, is_visible=True).all()
        subsections_data = []
        for sec in sections:
            meta = {}
        ...
        for q in ordered_questions:
            q_data = {
                ...
                'part': q.part,
                'mappedPart': 'part1' if q.part == 'reading' else ('part2' if q.part == 'writing' else q.part),
            ...
            if q.question_type == 'multiple_choice':
                ...
                if q.section_id:
                    sec = next((s for s in sections if s.section_id == q.section_id), None)
                ...
                if shuffle_choices and paper.randomization_enabled:
                    rng = random.Random(generated_paper.randomization_seed + q.question_id)
                    labels = [c['choice_label'] for c in choice_list]
                    rng.shuffle(choice_list)
                    for i, c in enumerate(choice_list):
                        c['choice_label'] = labels[i] if i < len(labels) else chr(65+i)
                ...
                if shuffle_items and paper.randomization_enabled:
                    rng = random.Random(generated_paper.randomization_seed + q.question_id + 1000)
        ...
        for r in saved_responses:
            if r.response_value is not None:
                rtype = r.response_type.name if hasattr(r.response_type, 'name') else str(r.response_type)
                if rtype == 'true_false':
                     try:
        ...
        return jsonify({
            ...
            'time_remaining_seconds': max(0, time_remaining_seconds), # GỬI SỐ GIÂY
...
def submit_response(attempt_id):
    ...
    try:
        ...
        # Sửa file client gọi API (Frontend) sẽ truyền lên field thứ 4 (dữ liệu thô) -> cần nhận ở backend
        ...
        # Nếu gửi lên không có question_id mà chỉ có mục đích chốt track thì kết thúc sớm.
        ...
        if question.question_type == 'short_answer' and not _is_valid_four_cell_answer(data.get('student_answer')):
...
def submit_exam(attempt_id):
    ...
    try:
        ...
        generated_paper = GeneratedPaper.query.get(attempt.generated_paper_id)
        has_essay = Question.query.filter_by(
...
def _assign_essay_graders(attempt):
    ...
    eligible_cross_school = [t.user_id for t in grader_teachers if t.school_id != attempt.student.school_id]
    
    # Nếu không đủ 2 người chéo trường, lấy tất cả giáo viên Ngữ Văn (chấp nhận chấm cùng trường)
    if len(eligible_cross_school) >= 2:
        final_eligible = eligible_cross_school
    else:
        final_eligible = [t.user_id for t in grader_teachers]
        
    # Nếu hệ thống thậm chí không có giáo viên Ngữ Văn nào, thì bỏ qua không tạo bản ghi rác
