...
def get_dashboard():
    ...
    try:
        ...
        schedules = schedules_response.get_json().get(
            'schedules', []) if status == 200 else []
                ...
                if schedule:
                    ...
                    if attempt:
                        if attempt.submitted_time:
                            status = 'completed'
                            score = float(
                                attempt.total_score) if attempt.total_score is not None else None
                        else:
                            status = 'ongoing'
...
def start_exam(schedule_id):
    ...
    try:
        ...
        generated_papers = GeneratedPaper.query.filter_by(
            paper_id=paper.paper_id, is_active=True).all()
...
def get_exam_paper(attempt_id):
    ...
    try:
        ...
        all_questions = Question.query.filter_by(
            paper_id=paper.paper_id, is_visible=True).all()
            ...
            if q.question_type == 'multiple_choice':
                ...
                if shuffle_choices and paper.randomization_enabled:
                    ...
                    for i, c in enumerate(choice_list):
                        c['choice_label'] = labels[i] if i < len(
                            labels) else chr(65 + i)
        ...
        for r in saved_responses:
            if r.response_value is not None:
                rtype = r.response_type.name if hasattr(
                    r.response_type, 'name') else str(
                    r.response_type)
                if rtype == 'true_false':
                    try:
