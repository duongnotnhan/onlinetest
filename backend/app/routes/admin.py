...
def approve_teacher(teacher_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Teacher approved successfully", "teacher_id": teacher_id}
            ),
            200,
        )
            ...
            schedule_data.append(
                {
                    ...
                    "exam_date": (
                        schedule.exam_date.isoformat() if schedule.exam_date else None
                    ),
                    "start_time": (
                        str(schedule.start_time) if schedule.start_time else None
                    ),
...
def publish_exam_session(session_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Exam session published", "exam_session_id": session_id}
            ),
            200,
        )
...
def approve_makeup_registration(reg_id):
    ...
    try:
        ...
        # Validate required fields
        if (
            not data.get("scheduled_date")
            or not data.get("scheduled_start_time")
            or not data.get("scheduled_end_time")
        ):
        ...
        if session and scheduled_date <= session.end_date:
            return (
                jsonify(
                    {
                        "error": "Makeup exam must be scheduled after the main exam session"
                    }
                ),
                400,
            )
        ...
        return (
            jsonify({"message": "Makeup exam approved", "registration_id": reg_id}),
            200,
        )
...
def reject_makeup_registration(reg_id):
    ...
    try:
        ...
        return (
            jsonify({"message": "Makeup exam rejected", "registration_id": reg_id}),
            200,
        )
...
def publish_exam_results(session_id):
    ...
    try:
        ...
        student_user_ids = (
            ...
            .join(ExamAttempt, Student.student_id == ExamAttempt.student_id)
            .filter(ExamAttempt.exam_session_id == session_id)
            .distinct()
            .all()
        )
                    ...
                    "data": [
                        {
                            ...
                            "session_name": (
                                schedule.exam_session.session_name
                                if schedule.exam_session
                                else None
                            ),
                            "subject_id": schedule.subject_id,
                            "subject_name": (
...
def create_school():
    ...
    try:
        ...
        if (
            not data.get("school_name")
            or not data.get("province_id")
            or not data.get("district_id")
        ):
            return (
                jsonify({"error": "Missing school_name, province_id, or district_id"}),
                400,
            )
...
def get_subjects():
    ...
    try:
        subjects = Subject.query.order_by(
            Subject.group_code.asc(), Subject.subject_name.asc()
        ).all()
        return (
            jsonify(
                {
...
def get_students():
    ...
    try:
        ...
        total = query.count()
        students = query.order_by(Student.full_name.asc()).paginate(
            page=page, per_page=limit, error_out=False
        )
        data = []
            ...
            data.append(
                {
                    ...
                    "school_name": (
                        _school_display_name(student.school)
...
def assign_grader():
    ...
    grade = EssayGrade.query.filter_by(
        response_id=response_id, grading_order=grading_order
    ).first()
