...
def grader_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if (
            ...
            or teacher.subject_specialty != "NGU_VAN_GRADER"
        ):
            ...
            jsonify(
                {
                    ...
                    "student_cccd": (
                        attempt.student.cccd if attempt and attempt.student else None
                    ),
                    "subject": (
                        attempt.subject.subject_name
                        if attempt and attempt.subject
                    ...
                    "current_score": (
                        float(essay.score) if essay.score is not None else None
                    ),
                    "grader_sequence": essay.grading_order,
                    "status": essay.final_status,
...
def _assign_reviewer_if_ready(response, current_grade):
    ...
    completed = EssayGrade.query.filter_by(
        response_id=response.response_id, final_status="completed"
    ).count()

    existing_reviewer = EssayGrade.query.filter_by(
        response_id=response.response_id, grading_order=3
    ).first()
    ...
    assigned_ids = [
        grade.grader_id
        for grade in EssayGrade.query.filter_by(response_id=response.response_id).all()
    ]
    ...
    candidates_cross = [
        t.user_id
        for t in grader_teachers
        if t.user_id not in assigned_ids
        and (not attempt or t.school_id != attempt.student.school_id)
    ]
...
def finalize_essay_scores(attempt_id):
    ...
    try:
        ...
        essay_responses = StudentResponse.query.filter_by(
            attempt_id=attempt_id, response_type="essay"
        ).all()
        ...
        for response in essay_responses:
            ...
            if not reviewer:
                return (
                    jsonify(
                        {
                            "error": "Reviewer grade is required before finalizing essay scores"
                        }
                    ),
                    400,
                )
            ...
            if reviewer.score is not None:
                ...
                if diff > 2.0:
                    ...
                    if len(grades) == 3:
                        avg_score = (
                            float(grades[0].score)
                            + float(grades[1].score)
                            + float(grades[2].score)
                        ) / 3
                    else:
                        # Average of 2
                        avg_score = (
