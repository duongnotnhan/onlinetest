...
def get_essays_for_grading():
    ...
    try:
        ...
        if exam_session_id:
            ...
            query = (
                query.join(StudentResponse, EssayGrade.response_id == StudentResponse.response_id)
...
def get_essay_details(essay_grade_id):
    ...
    try:
        ...
        answer_record = Answer.query.filter_by(question_id=question.question_id).first() if question else None
        ...
        return jsonify({
            ...
            'reading_material': reading_material # Trả về ngữ liệu (tùy chọn hiển thị)
        }), 200
...
def _assign_reviewer_if_ready(response, current_grade):
    ...
    assigned_ids = [
        grade.grader_id for grade in EssayGrade.query.filter_by(response_id=response.response_id).all()
    ]
    ...
    candidates_cross = [
        t.user_id for t in grader_teachers
        if t.user_id not in assigned_ids and (not attempt or t.school_id != attempt.student.school_id)
    ]
    
    if candidates_cross:
        chosen_grader_id = candidates_cross[0]
    else:
        # Dự phòng: Lấy GK3 cùng trường cũng được, nhưng tuyệt đối không trùng GK1, GK2
        candidates_same = [t.user_id for t in grader_teachers if t.user_id not in assigned_ids]
...
def finalize_essay_scores(attempt_id):
    ...
    try:
        ...
        for response in essay_responses:
            ...
            reviewer = next((grade for grade in grades if grade.grading_order == 3), None)
            if not reviewer:
                return jsonify({'error': 'Reviewer grade is required before finalizing essay scores'}), 400
            ...
            if reviewer.score is not None:
                ...
                if diff > 2.0:
                    ...
                    if len(grades) == 3:
                        avg_score = (float(grades[0].score) + float(grades[1].score) + float(grades[2].score)) / 3
                    else:
                        # Average of 2
