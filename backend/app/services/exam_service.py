...
class ExamPaperService:
            ...
            if question_type == 'multiple_choice':
                for idx, choice_data in enumerate(choices or [], start=1):
                    choice = AnswerChoice(
                        ...
                        choice_label=choice_data.get('choice_label') or choice_data.get('label') or chr(
                            64 + idx),
                        choice_text=choice_data.get('choice_text') or choice_data.get('text'),
                        display_order=choice_data.get(
                            'display_order',
                            idx))
    ...
    def generate_paper_versions(paper_id, num_versions=3):
        ...
        try:
            ...
            sections = QuestionSection.query.filter_by(
                paper_id=paper_id, is_visible=True).all()
            ...
            for version_num in range(num_versions):
                ...
                for part_name in sorted(questions_by_part.keys()):
                    ...
                    if paper.subject and paper.subject.subject_code == 'TIN_HO' and part_name == 'part2':
                        ...
                        for track_name in [
                            ...
                            if paper.randomization_enabled:
                                movable_qs = [
                                    q for q in track_qs if q.template_category != 'no_shuffle']
                                random.shuffle(movable_qs)
                                movable_iter = iter(movable_qs)
                                track_qs = [q if q.template_category == 'no_shuffle' else next(
                                    movable_iter) for q in track_qs]
                    ...
                    if paper.randomization_enabled:
                        movable_bids = [bid for bid in block_ids if block_meta[bid]
                                        ['type'] == 'subsection' or block_meta[bid]['shuffle']]
                    ...
                    for bid in shuffled_bids:
                        ...
                        if block_meta[bid]['type'] == 'subsection':
                            ...
                            if paper.randomization_enabled and block_meta[bid]['shuffle']:
                                movable_qs = [
                                    q for q in b_qs if q.template_category != 'no_shuffle']
                                random.shuffle(movable_qs)
                                movable_iter = iter(movable_qs)
                                b_qs = [
                                    q if q.template_category == 'no_shuffle' else next(movable_iter) for q in b_qs]
    ...
    def get_paper_details(paper_id):
        ...
        try:
            ...
            sections = QuestionSection.query.filter_by(
                paper_id=paper_id, is_visible=True).order_by(
            ...
            for q in questions:
                ...
                if q.question_type == 'true_false':
                    items = QuestionItem.query.filter_by(
                        question_id=q.question_id).order_by(
                        QuestionItem.display_order).all()
                    q_data['items'] = [
                        {
    ...
    def auto_submit_overdue_exams():
        ...
        try:
            ...
            ongoing_attempts = ExamAttempt.query.filter_by(
                status='ongoing', is_submitted=0).all()
            ...
            for attempt in ongoing_attempts:
                ...
                if attempt.end_time:
                    ...
                    cutoff_time = attempt.start_time + \
                        timedelta(minutes=attempt.schedule.duration_minutes)
...
def _score_response(attempt, question, response):
    ...
    if response.response_value is None or str(
            response.response_value).strip().lower() == 'null':
        return 0, False
    ...
    if question.question_type == 'multiple_choice':
        ...
        if shuffle_choices and paper and paper.randomization_enabled and generated_paper:
            ...
            rng = random.Random(
                generated_paper.randomization_seed +
        ...
        is_correct = bool(
            _normalize_answer(
                response.response_value) == _normalize_answer(expected_label))
        return (
            float(
    ...
    if question.question_type == 'short_answer':
        answer = Answer.query.filter_by(
            question_id=response.question_id).first()
        is_correct = bool(
            answer and _normalize_short_answer(
                response.response_value) == _normalize_short_answer(
                answer.answer_value))
        points = float(
            question.points or (
                0.5 if attempt.subject and attempt.subject.subject_code == 'TOAN' else 0.25))
...
def _score_true_false(question, response_value):
    import json
    try:
        answers = response_value if isinstance(
            response_value, dict) else json.loads(
            response_value or '{}')
    except Exception:
        answers = {}
