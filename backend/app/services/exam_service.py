...
class AuthService:
    ...
    @staticmethod
    def create_user(username, password, email=None, phone=None, full_name=None, role='student', school_id=None):
        """Create a new user"""
        user = User(
            username=username,
            email=email,
...
class StudentService:
    ...
    @staticmethod
    def register_student_for_exam(student_id, exam_session_id, subject_ids, registered_by=None):
...
class ExamService:
    ...
    @staticmethod
    def create_exam_session(session_name, session_type, start_date, end_date, created_by, description=None):
        """Create exam session"""
        session = ExamSession(
    ...
    @staticmethod
    def create_exam_schedule(exam_session_id, subject_id, exam_date, start_time, end_time):
...
class ExamPaperService:
    ...
    SUBJECT_STRUCTURES = {
        ...
        'Ngoại Ngữ': {
            'parts': {
                'Part 1': {'questions': 40, 'points_per_q': 0.25}
            },
            'total_questions': 40,
            'total_points': 10.0
        },
    ...
    @staticmethod
    def add_question(paper_id, question_type, question_text, part, question_number, 
                     points=None, created_by=None, choices=None, items=None,
                     answer_value=None, max_words=None, max_chars=None,
                     informatics_track=None, shuffle_enabled=True, subsection_id=None):
        ...
        try:
            ...
            if not points:
                points = _default_points_for_question(paper.subject, question_type, part)
            ...
            if question_type == 'multiple_choice':
                for idx, choice_data in enumerate(choices or [], start=1):
                    choice = AnswerChoice(
                        ...
                        choice_label=choice_data.get('choice_label') or choice_data.get('label') or chr(64 + idx),
                        choice_text=choice_data.get('choice_text') or choice_data.get('text'),
            ...
            if question_type == 'true_false':
                for idx, item_data in enumerate(items or [], start=1):
                    item = QuestionItem(
                        question_id=question.question_id,
                        item_label=item_data.get('item_label') or item_data.get('label') or str(idx),
                        item_text=item_data.get('item_text') or item_data.get('text'),
                        correct_value=str(item_data.get('correct_value')).lower()
                        if item_data.get('correct_value') is not None else None,
    ...
    def generate_paper_versions(paper_id, num_versions=3):
        ...
        try:
            ...
            sections = QuestionSection.query.filter_by(paper_id=paper_id, is_visible=True).all()
            ...
            for q in questions:
                ...
                mapped_part = q.part
                if mapped_part == 'reading': mapped_part = 'part1'
                elif mapped_part == 'writing': mapped_part = 'part2'
            ...
            for version_num in range(num_versions):
                ...
                # part1 -> part2 -> part3 HOẶC rc1 -> rc2 -> rf1 -> rf2 -> rf3 -> rs
                for part_name in sorted(questions_by_part.keys()):
                    part_qs = questions_by_part[part_name]
                    
                    if paper.subject and paper.subject.subject_code == 'TIN_HO' and part_name == 'part2':
                        tracks = {'common': [], 'computer_science': [], 'applied_informatics': []}
                        for q in part_qs:
                            track = q.informatics_track if q.informatics_track else 'common'
                            if track not in tracks:
                                track = 'common'
                            tracks[track].append(q)
                            
                        part_final_order = []
                        for track_name in ['common', 'computer_science', 'applied_informatics']:
                            track_qs = tracks[track_name]
                            if not track_qs: continue
                            
                            track_qs.sort(key=lambda q: q.question_number)
                            if paper.randomization_enabled:
                                movable_qs = [q for q in track_qs if q.template_category != 'no_shuffle']
                                random.shuffle(movable_qs)
                                movable_iter = iter(movable_qs)
                                track_qs = [q if q.template_category == 'no_shuffle' else next(movable_iter) for q in track_qs]
                    ...
                    for q in part_qs:
                        if q.section_id:
                            ...
                            if b_id not in blocks:
                                ...
                                sec_type = sec.section_type.section_type_key if sec and sec.section_type else 'normal'
                            ...
                            block_meta[b_id]['min_q'] = min(block_meta[b_id]['min_q'], q.question_number)
                        else:
                    ...
                    if paper.randomization_enabled:
                        movable_bids = [bid for bid in block_ids if block_meta[bid]['type'] == 'subsection' or block_meta[bid]['shuffle']]
                        random.shuffle(movable_bids)
                        movable_iter = iter(movable_bids)
                        shuffled_bids = [
                            bid if (block_meta[bid]['type'] == 'loose' and not block_meta[bid]['shuffle']) else next(movable_iter)
                    ...
                    for bid in shuffled_bids:
                        ...
                        if block_meta[bid]['type'] == 'subsection':
                            ...
                            if paper.randomization_enabled and block_meta[bid]['shuffle']:
                                movable_qs = [q for q in b_qs if q.template_category != 'no_shuffle']
                                random.shuffle(movable_qs)
                                movable_iter = iter(movable_qs)
                                b_qs = [q if q.template_category == 'no_shuffle' else next(movable_iter) for q in b_qs]
    ...
    def finalize_paper(paper_id):
        ...
        try:
            ...
            questions_without_answers = Question.query.filter_by(paper_id=paper_id).all()
            for q in questions_without_answers:
                ...
                if not answer and q.question_type == 'multiple_choice':
                    return False, f'Question {q.question_number} has no answer key'
                if q.question_type == 'short_answer' and not answer:
                    return False, f'Question {q.question_number} has no short answer key'
                if q.question_type == 'true_false' and q.items.count() != 4:
                    return False, f'Question {q.question_number} must have 4 true/false items'
    ...
    def get_paper_details(paper_id):
        from app.models import ExamPaper, Question, AnswerChoice, QuestionItem, Answer, GeneratedPaper, Subject, QuestionSection
        import json
        ...
        try:
            ...
            sections = QuestionSection.query.filter_by(paper_id=paper_id, is_visible=True).order_by(QuestionSection.display_order).all()
            subsections_data = []
            for sec in sections:
                meta = {}
                try:
                ...
                subsections_data.append({
                    ...
                    'type': sec.section_type.section_type_key if sec.section_type else 'normal',
            ...
            for q in questions:
                ...
                if q.question_type == 'true_false':
                    items = QuestionItem.query.filter_by(question_id=q.question_id).order_by(QuestionItem.display_order).all()
                    q_data['items'] = [
                        {
            ...
            return {
                ...
                'total_points': float(paper.total_points) if paper.total_points else 10.0,
...
def _normalize_question_part(part):
    ...
    mapping = {
        'phần i': 'part1', 'phan i': 'part1', 'part 1': 'part1', 'part1': 'part1',
        'phần ii': 'part2', 'phan ii': 'part2', 'part 2': 'part2', 'part2': 'part2',
        'phần iii': 'part3', 'phan iii': 'part3', 'part 3': 'part3', 'part3': 'part3',
        'reading': 'reading', 'writing': 'writing', 'essay': 'writing',
        # Tích hợp nhận diện bản đồ định danh môn Tiếng Anh:
        'rc1': 'rc1', 'rc2': 'rc2', 'rf1': 'rf1', 'rf2': 'rf2', 'rf3': 'rf3', 'rs': 'rs'
    }
    allowed_parts = {'part1', 'part2', 'part3', 'reading', 'writing', 'rc1', 'rc2', 'rf1', 'rf2', 'rf3', 'rs'}
...
def _score_response(attempt, question, response):
    ...
    if response.response_value is None or str(response.response_value).strip().lower() == 'null':
        return 0, False
    ...
    if question.question_type == 'multiple_choice':
        ...
        if shuffle_choices and paper and paper.randomization_enabled and generated_paper:
            ...
            rng = random.Random(generated_paper.randomization_seed + question.question_id)
        ...
        is_correct = bool(_normalize_answer(response.response_value) == _normalize_answer(expected_label))
    ...
    if question.question_type == 'short_answer':
        answer = Answer.query.filter_by(question_id=response.question_id).first()
        is_correct = bool(answer and _normalize_short_answer(response.response_value) == _normalize_short_answer(answer.answer_value))
        points = float(question.points or (0.5 if attempt.subject and attempt.subject.subject_code == 'TOAN' else 0.25))
...
def _score_true_false(question, response_value):
    import json
    try:
        answers = response_value if isinstance(response_value, dict) else json.loads(response_value or '{}')
    except Exception:
        answers = {}
