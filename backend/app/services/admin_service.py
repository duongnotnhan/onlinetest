...
class AdminService:
    ...
    @staticmethod
    def approve_makeup_exam(registration_id, scheduled_date, start_time, end_time, approved_by):
...
class ExamStatisticsService:
    ...
    def get_session_statistics(session_id):
        ...
        try:
            total_attempts = ExamAttempt.query.filter_by(exam_session_id=session_id).count()
            completed = ExamAttempt.query.filter_by(exam_session_id=session_id, status='graded').count()
            pending_grading = ExamAttempt.query.filter_by(exam_session_id=session_id, status='completed').count()
            ...
            avg_score = 0
            if graded:
                total_score = sum(float(a.total_score) if a.total_score else 0 for a in graded)
    ...
    def get_subject_statistics(session_id, subject_id):
        ...
        try:
            ...
            total = len(attempts)
            passed = sum(1 for a in attempts if a.total_score and float(a.total_score) >= 5.0)
            failed = total - passed
            
            avg_score = 0
            if total > 0:
                total_score = sum(float(a.total_score) if a.total_score else 0 for a in attempts)
