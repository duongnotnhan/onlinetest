...
class School(db.Model):
    ...
    province_id = db.Column(db.Integer, db.ForeignKey('provinces.province_id'), nullable=True)
...
class ExamSchedule(db.Model):
    ...
    exam_session_id = db.Column(db.Integer, db.ForeignKey('exam_sessions.exam_session_id'), nullable=False)
...
class ExamPaper(db.Model):
    ...
    subject_id = db.Column(db.Integer, db.ForeignKey('subjects.subject_id'), nullable=False)
    exam_session_id = db.Column(db.Integer, db.ForeignKey('exam_sessions.exam_session_id'))
    ...
    reading_material = db.Column(db.Text, nullable=True, comment='Shared reading material for Vietnamese Language (Ngữ Văn)')
    created_by = db.Column(db.Integer, db.ForeignKey('users.user_id'), nullable=False)
    approved_by = db.Column(db.Integer, db.ForeignKey('users.user_id'))
    approval_date = db.Column(db.DateTime)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Question(db.Model):
    ...
    paper_id = db.Column(db.Integer, db.ForeignKey('exam_papers.paper_id'), nullable=False)
    ...
    part = db.Column(db.Enum('part1', 'part2', 'part3', 'reading', 'writing', 'rc1', 'rc2', 'rf1', 'rf2', 'rf3', 'rs'), nullable=False)
    
    points = db.Column(db.Numeric(5, 2))
    max_words = db.Column(db.Integer)
    max_chars = db.Column(db.Integer)
    answer_format = db.Column(db.Enum('choice', 'true_false_set', 'four_cell_text', 'essay_text'), nullable=True)
    informatics_track = db.Column(db.Enum('common', 'computer_science', 'applied_informatics'), nullable=True)
    ...
    section_id = db.Column(db.Integer, db.ForeignKey('question_sections.section_id'), nullable=True)
    passage_id = db.Column(db.Integer, db.ForeignKey('question_passages.passage_id'), nullable=True)
    display_order_in_section = db.Column(db.Integer)
    is_rich_text = db.Column(db.Boolean, default=True)
    difficulty_level = db.Column(db.Enum('easy', 'medium', 'hard', 'very_hard'), default='medium')
    bloom_level = db.Column(db.Enum('remember', 'understand', 'apply', 'analyze', 'evaluate', 'create'), default='understand')
    estimated_time_seconds = db.Column(db.Integer, default=60)
    is_template = db.Column(db.Boolean, default=False)
    template_category = db.Column(db.String(100))
    
    choices = db.relationship('AnswerChoice', backref='question', lazy='dynamic', cascade='all, delete-orphan')
    items = db.relationship('QuestionItem', backref='question', lazy='dynamic', cascade='all, delete-orphan')
...
class AnswerChoice(db.Model):
    ...
    # Rich Text và Media
    is_rich_text = db.Column(db.Boolean, default=True)
    media_id = db.Column(db.Integer, db.ForeignKey('question_media.media_id'), nullable=True)
...
class Answer(db.Model):
    ...
    question_id = db.Column(db.Integer, db.ForeignKey('questions.question_id'), nullable=False)
    answer_type = db.Column(db.Enum('choice', 'true_false', 'text', 'numeric'), nullable=False)
...
class ExamAttempt(db.Model):
    ...
    schedule_id = db.Column(db.Integer, db.ForeignKey('exam_schedules.schedule_id'), nullable=False)
    generated_paper_id = db.Column(db.Integer, db.ForeignKey('generated_papers.generated_paper_id'), nullable=False)
    exam_session_id = db.Column(db.Integer, db.ForeignKey('exam_sessions.exam_session_id'), nullable=False)
    subject_id = db.Column(db.Integer, db.ForeignKey('subjects.subject_id'), nullable=False)
    ...
    selected_informatics_track = db.Column(db.Enum('computer_science', 'applied_informatics'), nullable=True)
    status = db.Column(db.Enum('ongoing', 'completed', 'graded', 'invalid'), default='ongoing')
    ...
    subject = db.relationship('Subject')
    responses = db.relationship('StudentResponse', backref='attempt', lazy='dynamic', cascade='all, delete-orphan')
...
class StudentResponse(db.Model):
    ...
    question_id = db.Column(db.Integer, db.ForeignKey('questions.question_id'), nullable=False)
    response_type = db.Column(db.Enum('choice', 'true_false', 'text', 'numeric', 'essay'), nullable=False)
...
class EssayGrade(db.Model):
    ...
    response_id = db.Column(db.Integer, db.ForeignKey('student_responses.response_id'), nullable=False)
    ...
    final_status = db.Column(db.Enum('pending', 'grading', 'completed'), default='pending')
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class SpecialQuestionGroup(db.Model):
    ...
    question_id = db.Column(db.Integer, db.ForeignKey('questions.question_id'), nullable=False)
    group_type = db.Column(db.Enum('common', 'computer_science', 'applied_informatics'), nullable=False)
...
class QuestionSectionType(db.Model):
    ...
    sections = db.relationship('QuestionSection', backref='section_type', lazy='dynamic')
...
class QuestionSection(db.Model):
    ...
    paper_id = db.Column(db.Integer, db.ForeignKey('exam_papers.paper_id'), nullable=False)
    section_type_id = db.Column(db.Integer, db.ForeignKey('question_section_types.section_type_id'), nullable=True)
    ...
    passages = db.relationship('QuestionPassage', backref='section', lazy='dynamic', cascade='all, delete-orphan')
    questions = db.relationship('Question', backref='section', lazy='dynamic', foreign_keys='Question.section_id')
...
class QuestionPassage(db.Model):
    ...
    creator = db.relationship('User', foreign_keys=[created_by])
    media_items = db.relationship('QuestionMedia', backref='passage', lazy='dynamic',
                                   foreign_keys='QuestionMedia.passage_id')
    questions = db.relationship('Question', backref='passage_ref', lazy='dynamic',
...
class QuestionMedia(db.Model):
    ...
    section_id = db.Column(db.Integer, db.ForeignKey('question_sections.section_id'), nullable=True)
    passage_id = db.Column(db.Integer, db.ForeignKey('question_passages.passage_id'), nullable=True)
    media_type = db.Column(db.Enum('image', 'audio', 'video', 'document'), default='image')
...
class QuestionMultiSection(db.Model):
    ...
    question_id = db.Column(db.Integer, db.ForeignKey('questions.question_id'), nullable=False)
    section_id = db.Column(db.Integer, db.ForeignKey('question_sections.section_id'), nullable=False)
    display_order = db.Column(db.Integer)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
